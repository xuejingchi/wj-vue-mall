  ...
  <section class="identify-check-page" v-loading.fullscreen.lock="loading">
    ...
    <ul class="check-form-list undeline">
      <li v-for="(item, index) in listItem" v-if="index < listItem.length - 1" class="check-form-item">
        ...
        <select ref="supplierTypeSelect" v-if="item.type === 'select'" v-model="temp.supplierType" class="form-item-input form-item-select">
          <option v-for="type in supplierTypeList" :value="type.id">{{ type.name }}</option>
        </select>
...
<script>
  import { getCompanyTypeList, supplierAuthentication } from '@/api/person/identification'
    ...
    data () {
      ...
      return {
        ...
        listItem: [
          {
            ...
            key: 'businessArea',
            type: 'input',
            placeholder: '省份+城市+县/区',
            required: true,
            rule: value => {
              return baseRule('经营区域', 60)(value)
            }
          }, {
            name: '经营品类',
            key: 'businessCategory',
            type: 'input',
            placeholder: '如洁具，钢构件等',
            required: true,
            rule: value => {
              return baseRule('经营品类', 60)(value)
            }
          }, {
            name: '主营产品',
            key: 'mainProduct',
            type: 'textarea',
            placeholder: '公司主营产品信息，多个产品，逗号分隔填写',
            required: true,
            rule: value => {
              return baseRule('主营产品', 200)(value)
            }
          }, {
            name: '品牌',
            key: 'brand',
            type: 'input',
            placeholder: '如经营多个品牌，逗号分隔填写',
            required: true,
            rule: value => {
              return baseRule('品牌', 60)(value)
            }
          }, {
            name: '统一社会信用代码',
            key: 'newCreditCode',
        ...
        oldCerRule: {
          ...
          key: 'oldCreditCode',
          type: 'input',
          placeholder: '输入15位统一社会信用代码',
          required: true,
          rule: value => {
            if (!value || value.length !== 15) {
              return '请输入正确15位统一社会信用代码'
            }
          }
        }
      }
    ...
    methods: {
      ...
      * */
      loadData () {
        ...
        getCompanyTypeList({
          ...
          // 默认选中第一条内容
          this.$nextTick(() => {
            this.temp.supplierType = this.$refs.supplierTypeSelect[0].childNodes[0].value
            this.$refs.supplierTypeSelect[0].value = this.$refs.supplierTypeSelect[0].childNodes[0].value
          })
        }, err => {
          console.log('err', err)
        })
      ...
      * */
      newFileChanged (event, index) {
        ...
        this.newLicenceList[index].imgName = file.name
      ...
      * */
      doSubmit () {
        ...
        this.loading = true
        supplierAuthentication(query).then(res => {
          this.loading = false
          if (res.data.code === 200) {
            this.$message({
              message: '提交认证信息成功！',
