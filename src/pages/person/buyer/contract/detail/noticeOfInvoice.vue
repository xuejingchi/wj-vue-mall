      ...
      <div class="notice-info-item">
        <div class="notice-info-title">
          ...
          <div class="notice-info-title-item t-c">
            通知时间：{{ listData.deliverNoticeDate | timeFormat('YYYY-MM-DD') }}
          </div>
        ...
        <div class="notice-info-content clearfix">
          ...
          <div class="notice-info-content-txt">
            <div class="notice-info-txt-item clearfix" style="margin: 0">
              <div class="notice-info-txt-label">采购订单号</div>
              <div class="notice-info-txt-row">{{ listData.contractCode }}</div>
            </div>
            ...
            <div class="notice-info-txt-item clearfix">
              <div class="notice-info-txt-label">项目名称</div>
              <div class="notice-info-txt-row">{{ listData.projectName }}</div>
            </div>
            <div class="notice-info-txt-item clearfix">
            ...
            <div class="notice-info-txt-item clearfix">
              <div class="notice-info-txt-label">销售经理</div>
              <div class="notice-info-txt-row">{{ listData.sales }}</div>
            </div>
          </div>
        </div>
      </div>
...
<script type="text/ecmascript-6">
  import { getDeliveryNoticeInfo } from '@/api/person/buyer/contract'
  ...
  export default {
    ...
    mounted () {
      this.getDeliveryNoticeInfo()
    },
    methods: {
      getDeliveryNoticeInfo() {
        let params = {
          deliveryNoticeId: this.$route.query.deliveryNoticeId,
          userId: this.userInfo.userId
        }
        getDeliveryNoticeInfo(params).then((response) => {
          if (response.data.code === 200) {
            let data = response.data.data
            this.listData = data
            this.tableData = data.itemList
            const params = {
              billId: this.$route.query.deliveryNoticeId,
              billType: '15',
